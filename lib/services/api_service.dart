...
class ApiService {
  ...
  Future<List<HouseTrading>> getHouseTrading(String symbol) async {
    try {
      debugPrint('🏠 Fetching house trading data for $symbol...');

      final url = Uri.parse(
        'https://financialmodelingprep.com/stable/house-trades?symbol=$symbol&apikey=$_apiKey',
      );

      final response = await http.get(url);

      if (response.statusCode != 200) {
        debugPrint('⚠️ House trading data not available for $symbol');
        return [];
      }
      ...
      if (data.isEmpty) {
        debugPrint('⚠️ No house trading data available for $symbol');
        return [];
      }

      final houseTrades = data
          .map((item) => HouseTrading.fromJson(item))
          .toList();
      ...
      debugPrint(
        '🏠 Loaded ${houseTrades.length} house trading records for $symbol',
      );
      return houseTrades;
    } catch (e) {
...
class HouseTrading {
  ...
  factory HouseTrading.fromJson(Map<String, dynamic> json) {
    return HouseTrading(
      symbol: json['symbol']?.toString() ?? '',
      disclosureDate: json['disclosureDate']?.toString() ?? '',
      transactionDate: json['transactionDate']?.toString() ?? '',
  ...
  @override
  String toString() {
    return 'HouseTrading(symbol: $symbol, name: $fullName, type: $type, amount: $amount)';
  }
}
