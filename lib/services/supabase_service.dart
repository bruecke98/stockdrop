...
class SupabaseService {
  ...
  // ==================== INSIGHTS NOTES MANAGEMENT ====================

  /// Save a user note for a specific insight type and stock symbol
  ///
  /// [symbol] - The stock symbol
  /// [insightType] - Type of insight (e.g., 'esg', 'analyst', 'institutional')
  /// [note] - The user's note content
  /// Returns true if successful, throws exception on error
  Future<bool> saveInsightNote(
    String symbol,
    String insightType,
    String note,
  ) async {
    try {
      if (!isAuthenticated) {
        throw Exception('User must be authenticated to save notes');
      }

      // Upsert the note (insert or update if exists)
      await client.from('st_insights_notes').upsert({
        'user_id': currentUserId,
        'symbol': symbol.toUpperCase(),
      ...
      debugPrint('Successfully saved insight note for $symbol ($insightType)');
      return true;
    } on PostgrestException catch (e) {
      if (e.code == '23505') {
        // Unique constraint violation, try updating instead
        await client
            .from('st_insights_notes')
            .update({
              'note': note,
              'updated_at': DateTime.now().toIso8601String(),
            })
      ...
      debugPrint('Error saving insight note: ${e.message}');
      throw Exception('Failed to save insight note: ${e.message}');
    } catch (e) {
      debugPrint('Error saving insight note: $e');
      throw Exception('Failed to save insight note: $e');
    }
  }

  /// Get all user notes for a specific stock symbol
  ///
  /// [symbol] - The stock symbol
  /// Returns Map of insight type to note content
  Future<Map<String, String>> getInsightNotes(String symbol) async {
    try {
      if (!isAuthenticated) {
        return {};
      }
      ...
      final response = await client
          .from('st_insights_notes')
          .select('insight_type, note')
          .eq('user_id', currentUserId)
          .eq('symbol', symbol.toUpperCase());

      final notes = <String, String>{};
      for (final row in response) {
        final insightType = row['insight_type'] as String;
        final note = row['note'] as String;
        if (note.isNotEmpty) {
          notes[insightType] = note;
        }
      }

      debugPrint('Retrieved ${notes.length} insight notes for $symbol');
      return notes;
    } on PostgrestException catch (e) {
      debugPrint('Error getting insight notes: ${e.message}');
      return {};
    } catch (e) {
      debugPrint('Error getting insight notes: $e');
      return {};
    }
  }
  ...
  ///
  /// [symbol] - The stock symbol
  /// [insightType] - Type of insight to delete
  /// Returns true if successful, throws exception on error
  Future<bool> deleteInsightNote(String symbol, String insightType) async {
    try {
      if (!isAuthenticated) {
        throw Exception('User must be authenticated to delete notes');
      }
      ...
      debugPrint(
        'Successfully deleted insight note for $symbol ($insightType)',
      );
      return true;
    } on PostgrestException catch (e) {
      debugPrint('Error deleting insight note: ${e.message}');
      throw Exception('Failed to delete insight note: ${e.message}');
    } catch (e) {
      debugPrint('Error deleting insight note: $e');
      throw Exception('Failed to delete insight note: $e');
    }
  }

  /// Get all notes for the current user (for export/backup purposes)
  ///
  /// Returns Map of symbol -> Map of insight type -> note
  Future<Map<String, Map<String, String>>> getAllInsightNotes() async {
    try {
      if (!isAuthenticated) {
        return {};
      }
      ...
      final response = await client
          .from('st_insights_notes')
          .select('symbol, insight_type, note')
          .eq('user_id', currentUserId);

      final allNotes = <String, Map<String, String>>{};
      for (final row in response) {
        final symbol = row['symbol'] as String;
        final insightType = row['insight_type'] as String;
        final note = row['note'] as String;

        if (!allNotes.containsKey(symbol)) {
          allNotes[symbol] = <String, String>{};
        }

        if (note.isNotEmpty) {
          allNotes[symbol]![insightType] = note;
        }
      }

      debugPrint('Retrieved insight notes for ${allNotes.length} symbols');
      return allNotes;
    } on PostgrestException catch (e) {
      debugPrint('Error getting all insight notes: ${e.message}');
      return {};
    } catch (e) {
      debugPrint('Error getting all insight notes: $e');
      return {};
    }
  }
