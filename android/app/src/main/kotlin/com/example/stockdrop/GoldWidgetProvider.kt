...
private fun updateWidgetWithGoldData(
    ...
    // Set change color
    val changeColor = if (quote.changePercent >= 0) {
        Color.parseColor("#4CAF50") // Green
    } else {
        Color.parseColor("#F44336") // Red
    }
    views.setTextColor(R.id.gold_change, changeColor)
    
    // Generate and set chart
    if (historicalData.isNotEmpty()) {
        val chartBitmap = generateChartBitmap(historicalData, quote.changePercent >= 0)
        views.setImageViewBitmap(R.id.gold_chart, chartBitmap)
    }
    
    appWidgetManager.updateAppWidget(appWidgetId, views)
}

private fun generateChartBitmap(data: List<GoldHistoricalPoint>, isPositive: Boolean): Bitmap {
    val width = 300
    val height = 80
    ...
    val paint = Paint().apply {
        ...
        color = if (isPositive) Color.parseColor("#4CAF50") else Color.parseColor("#F44336")
    }
    
    val fillPaint = Paint().apply {
        isAntiAlias = true
        style = Paint.Style.FILL
        color = if (isPositive) Color.parseColor("#2E4CAF50") else Color.parseColor("#2EF44336")
    }
    ...
    for (i in data.indices) {
        ...
        val y = height - ((data[i].close - minPrice) / priceRange * height).toFloat()
    ...
    if (points.isNotEmpty()) {
        fillPath.lineTo(points[points.size - 2], height.toFloat())
        fillPath.lineTo(points[0], height.toFloat())
        fillPath.close()
